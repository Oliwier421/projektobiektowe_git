...
void Operator::otworzWyciagOP(Stacja& s, int id) {
    ...
    if (ID!=-1) s.wyciagi[ID] -> otworzWyciag();
    else std::cout << "Nie znaleziono wyciagu o takim id.\n";
}

void Operator::zamknijWyciagOP(Stacja& s, int id) {
    int ID = znajdzWyciag(s.wyciagi, id);
    if (ID!=-1) s.wyciagi[ID] -> zamknijWyciag(s.z.getSec());
    else std::cout << "Nie znaleziono wyciagu o takim id.\n";
}
...
void Operator::zatrzymajWyciagOP(Stacja& s, int id, int czas){
    ...
    int ID = znajdzWyciag(s.wyciagi, id);
    if (ID!=-1) s.wyciagi[ID] -> zatrzymajWyciag(czas);
    else std::cout << "Nie znaleziono wyciagu o takim id.\n";
}
...
void Operator::usunWyciag(Stacja& s, int id){
    int ID = znajdzWyciag(s.wyciagi, id);
    if (ID!=-1) s.wyciagi.erase(s.wyciagi.begin()+id-1);
    else std::cout << "Nie znaleziono wyciagu o takim id.\n";
}
...
void Operator::menu(Stacja& s) {
    ...
    while (on) {
        ...
        switch (i){
            ...
            case 5:{
                int id;
                std::cout << "\nWpisz id wyciagu, który chcesz usunac: ";
                std::cin >> id; std::cin.ignore();
                usunWyciag(s, id);
                break;}
            case 6:
